...
class PSCMaps {
 public:
  ...
  inline bool
  is_partially_visible();      // true iif  -alpha <= beta <= alpha (sphere partially visible)
  inline bool is_invisible();  // true iif  beta <= -alpha <= 0 (sphere fully invisible)
  inline bool
  is_center_below_hor();  // true iif  beta <= 0 (spherical cap center is below horizon)
  ...
  inline T
  get_phi_l();  // phi_l: tangency points angle (only when partially visible and using radial map)
  ...
  T eval_Ar_inverse(
      T Ar_value) const;  // evals. Ar inverse (Ar in [0,area/2]), iteratively (when needed)
...
template<class T> class Vars {
 public:
  static T iN_tolerance;    // tolerance for inverse newton....
  static int iN_max_iters;  // max iters. for inv. Newton
  static bool trace_newton_inversion;
...
template<class T> T eval_I(T u, T w);
...
template<class T>
T InverseNSB(FuncType<T> F, FuncType<T> f, const T t_max, const T Aobj, const T A_max);

// ---------------------------------------------------------------------
// numerically integrate a real function on a real interval (x0,x1),
// by using 'n' equispaced samples

template<class T> T numeric_integral(const std::function<T(T)> &f, T x0, T x1, int n);
...
template<class T> inline T PSCMaps<T>::get_area()
{
  ensure_initialized();
  return F;
}
...
template<class T> inline T PSCMaps<T>::get_xe()
{
  ensure_initialized();
  return xe;
}
...
template<class T> inline T PSCMaps<T>::get_ax()
{
  ensure_initialized();
  return ax;
}
...
template<class T> inline T PSCMaps<T>::get_ay()
{
  ensure_initialized();
  return ay;
}
...
template<class T> inline T PSCMaps<T>::get_xl()
{
  ensure_initialized();
  return xl;
}
...
template<class T> inline T PSCMaps<T>::get_yl()
{
  ensure_initialized();
  return yl;
}
...
template<class T> inline T PSCMaps<T>::get_phi_l()
{
  ensure_initialized();
  if (do_checks)
...
template<class T> inline bool PSCMaps<T>::is_initialized()
{
  return initialized;
}
// -------------------------------------------------------------------------

template<class T> inline bool PSCMaps<T>::is_invisible()
{
  ensure_initialized();
  return invisible;
}
...
template<class T> inline bool PSCMaps<T>::is_partially_visible()
{
  ensure_initialized();
...
template<class T> inline bool PSCMaps<T>::is_fully_visible()
{
  ensure_initialized();
...
template<class T> inline bool PSCMaps<T>::is_center_below_hor()
{
  ensure_initialized();
...
template<class T> inline bool PSCMaps<T>::is_using_radial()
{
  ensure_initialized();
  return using_radial;
}
...
template<class T> PSCMaps<T>::PSCMaps()
{
  initialized = false;
  E = 0.0;
  L = 0.0;
...
template<class T> inline void PSCMaps<T>::ensure_initialized()
{
  if (do_checks)
    assert(initialized);
}
...
template<class T> inline void PSCMaps<T>::ensure_using_radial()
{
  if (do_checks) {
    assert(initialized);
    assert(!invisible);
...
template<class T> inline void PSCMaps<T>::ensure_using_parallel()
{
  if (do_checks) {
    assert(initialized);
    assert(!invisible);
...
template<class T>
void PSCMaps<T>::initialize(const T p_alpha, const T p_beta, const bool p_use_radial)
{
  constexpr T tolerance = 1e-5, pi2 = T(M_PI) * T(0.5);
  ...
  sin_beta = std::sin(beta), sin_beta_abs = std::abs(sin_beta);
...
template<class T> inline void PSCMaps<T>::compute_ELF_xlyl_phi_l()
{
  // initialize areas
  E = 0.0;
  L = 0.0;
...
template<class T> inline void PSCMaps<T>::check_y(T &y, const T &max_y) const
{
  if (do_checks) {
    assert(initialized);
    assert(!invisible);
...
template<class T> T PSCMaps<T>::eval_xEll(T y) const
{
  if (do_checks)
...
template<class T> T PSCMaps<T>::eval_xCir(T y) const
{
  if (do_checks)
...
template<class T> T PSCMaps<T>::eval_ApE(T y) const
{
  if (do_checks)
    assert(initialized && !using_radial);
...
template<class T> T PSCMaps<T>::eval_ApC(T y) const
{
  if (do_checks)
    assert(initialized && !using_radial && partially_visible);
...
template<class T> T PSCMaps<T>::eval_Ap(T y) const
{
  if (do_checks) {
    assert(initialized);
    assert(!invisible);
  ...
  // lune only
  if (center_below_hor)
    return (y <= yl) ? (eval_ApC(y) - eval_ApE(y)) : L;
...
template<class T> T PSCMaps<T>::eval_par_integrand(T y) const
{
  if (do_checks) {
    assert(initialized);
    assert(!invisible);
  ...
  // lune only
  if (center_below_hor)
    return (y <= yl) ? (eval_xCir(y) - eval_xEll(y)) : T(0.0);

  // ellipse+lune
  return (y <= yl) ? eval_xCir(y) + eval_xEll(y) : T(2.0) * eval_xEll(y);
}

// --------------------------------------------------------------------------
// for a given y, eval x0 and x1
// y must be in (0,ay), but if center is below horizon, it must be in (0,yl)
template<class T> void PSCMaps<T>::eval_xmin_xmax(const T y, T &xmin, T &xmax) const
{
  T yy = y;
  if (do_checks) {
    assert(initialized && !using_radial);
...
template<class T> T PSCMaps<T>::eval_Ap_inverse(T Ap_value) const
{
...
template<class T> void PSCMaps<T>::hor_map(T s, T t, T &x, T &y) const
{
  ...
  const float u = y_is_neg ? T(1.0) - T(2.0) * t : T(2.0) * t - T(1.0);
...
template<class T> inline void PSCMaps<T>::check_theta(T &theta, const T &max_theta) const
{
  if (do_checks) {
    assert(initialized);
    assert(using_radial);
...
template<class T> T PSCMaps<T>::eval_rEll(T theta) const  // theta in [0,pi/2]
{
  if (do_checks)
    assert(initialized && using_radial);
  ...
  const T sin_theta = sin(theta), sin_theta_sq = sin_theta * sin_theta;
...
template<class T> T PSCMaps<T>::eval_rCirc(T theta) const  // theta in [0,phi_l]
{
  if (do_checks)
    assert(initialized && using_radial && partially_visible);
  ...
  const T sin_theta = sin(theta), sin_theta_sq = sin_theta * sin_theta,
          cos_theta = std::sqrt(T(1.0) -
                                sin_theta_sq);  // sign is positive because theta < phi_l < PI/2
  return std::sqrt(T(1.0) - xe_sq * sin_theta_sq) - xe * cos_theta;
}
// ---------------------------------------------------------------------------
// evaluate Re according to expression 50  (sec.4)
// theta in [0,pi]
template<class T> T PSCMaps<T>::eval_ArE(T theta) const
{
...
template<class T> T PSCMaps<T>::eval_ArC(T theta) const
{
  if (do_checks)
    assert(initialized && using_radial && partially_visible);
  ...
  return T(0.5) * (theta - std::asin(xez) + xe_sq * z * std::sqrt(T(1.0) - z_sq) -
...
template<class T> T PSCMaps<T>::eval_Ar(T theta) const
{
  if (do_checks) {
    assert(initialized);
    assert(!invisible);
  ...
  // lune only
  if (center_below_hor)
    return (theta <= phi_l) ? (eval_ArC(theta) - eval_ArE(theta)) : L;
...
template<class T> T PSCMaps<T>::eval_rad_integrand(T theta) const
{
  if (do_checks) {
    assert(initialized);
    assert(!invisible);
  ...
  if (center_below_hor) {
    if (theta <= phi_l) {
      const T rc = eval_rCirc(theta), re = eval_rEll(theta);
...
template<class T> void PSCMaps<T>::eval_rmin_rmax(const T theta, T &rmin, T &rmax) const
{
  if (do_checks) {
    assert(initialized);
    assert(!invisible);
...
template<class T> T PSCMaps<T>::eval_Ar_inverse(T Ar_value) const
{
  ...
  const T theta_max = center_below_hor ? phi_l : T(M_PI),
          theta_result = InverseNSB<T>(Ar_func, Ar_integrand, theta_max, A_frac, T(1.0));
...
//
template<class T> T PSCMaps<T>::eval_ArE_inverse(T Ar_value) const
{
...
template<class T> void PSCMaps<T>::rad_map(T s, T t, T &x, T &y) const
{
  ...
  const float u = angle_is_neg ? T(1.0) - T(2.0) * t : T(2.0) * t - T(1.0);
  ...
  {
    varphi = std::max(T(0.0), std::min(T(M_PI), eval_Ar_inverse(u * T(0.5) * F)));
  ...
  const T si = angle_is_neg ? -(std::sin(varphi)) : std::sin(varphi),
      ...
      rad = std::sqrt(s * (rmax * rmax) + (T(1.0) - s) * (rmin * rmin)), xp = rad * co,
          yp = rad * si;
...
template<class T> void PSCMaps<T>::eval_map(T s, T t, T &x, T &y) const
{
  if (do_checks)
    assert(initialized);
...
{
  const std::string T_descr = std::is_same<T, float>::value ?
                                  "float" :
                                  std::is_same<T, double>::value ? "double" : "other";
...
T InverseNSB(FuncType<T> F, FuncType<T> f, const T t_max, const T Aobj, const T A_max)
{
  ...
  if (do_checks)
    if (Vars<T>::trace_newton_inversion) {
      ...
      cout << endl
           << "begins InverseNSB, t_max = " << t_max << ", Aobj == " << Aobj
           << ", A_max == " << A_max << endl
           << endl;
    }
  ...
  const T A = std::max(T(0.0), std::min(Aobj, A_max));  // A is always positive
  T tn = (A / A_max) * t_max,  // current best estimation of result value 't'
      tn_min = T(0.0),         // current interval: minimum value
      tn_max = t_max,          // current interval: maximum value
      diff_tn_min =
          T(0.0) -
          A,  // == F(tn_min) - A, (always negative) current difference at the left extreme of the interval
      diff_tn_max =
          A_max -
          A;  // == F(tn_max) - A, (always positive) current difference at the right extreme of the interval
  ...
  while (true) {
    if (do_checks)
      if (Vars<T>::trace_newton_inversion) {
        if (num_iters < 10)
          cout << " ";
        cout << "  (" << num_iters << "): tn_min " << tn_min << ", tn " << tn << ", tn_max "
             << tn_max;
      }
    const T Ftn = F(tn);
    ...
    if (std::abs(diff) <= Vars<T>::iN_tolerance) {
      if (do_checks)
        if (Vars<T>::trace_newton_inversion)
          cout << endl
               << endl
               << "end iterations, diff <= tolerance == " << Vars<T>::iN_tolerance << endl;
      break;
    }
    ...
    if (Vars<T>::iN_max_iters < num_iters) {
      if (do_checks)
        if (Vars<T>::trace_newton_inversion)
          cout << endl
               << "end iterations: num_iters exceeded max ==  " << Vars<T>::iN_max_iters << endl;
      break;
    }
  }
...
template<class T> void PSCMaps<T>::run_test_integrals()
{
  using namespace std;
  ...
  if (partially_visible) {
    ...
    if (!using_radial) {
      ...
      auto integrand_h = [&](T y) { return eval_xCir(y) - eval_xEll(y); };
      ...
      auto integrand_r = [&](T theta) {
        const T re = eval_rEll(theta), rc = eval_rCirc(theta);
...
template<class T> void Vars<T>::print_settings()
{
  const std::string T_descr = std::is_same<T, float>::value ?
                                  "float" :
                                  std::is_same<T, double>::value ? "double" : "other";
