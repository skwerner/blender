...
const inline int compute_index(
    const int x, const int y, const int z, const int width, const int height, const int depth)
{
...
const inline int compute_index(const int x, const int y, const int z, const int3 resolution)
{
  return compute_index(x, y, z, resolution.x, resolution.y, resolution.z);
}

const inline int compute_index(
    const size_t x, const size_t y, const size_t z, const size_t width, const size_t height)
{
  return x + width * (y + z * height);
}
                               ...
                               const int depth)
{
   ...
   * voxel in tile space. */
  int tix = x >> TILE_INDEX_SHIFT, itix = x & TILE_INDEX_MASK, tiy = y >> TILE_INDEX_SHIFT,
      itiy = y & TILE_INDEX_MASK, tiz = z >> TILE_INDEX_SHIFT, itiz = z & TILE_INDEX_MASK;
  /* Get flat index of voxel's tile. */
  int tile = compute_index(
      tix, tiy, tiz, get_tile_res(width), get_tile_res(height), get_tile_res(depth));
  if (tile < 0) {
    return -1;
  }
                        ...
                        vector<int> *grid_info)
{
  ...
  const int tile_count = get_tile_res(width) * get_tile_res(height) * get_tile_res(depth);
  ...
  if (sparse_mem_use >= dense_mem_use) {
    VLOG(1) << "Memory of " << grid_name << " increased from "
            ...
            << string_human_readable_size(sparse_mem_use) << ", not using sparse grid";
    return false;
  }
...
const static int2 padded_tile_bound(const vector<int> *offsets,
                                    const int x,
                                    const int width,
                                    const int tile)
{
                            ...
                            int3 &sparse_resolution)
{
  ...
  if (!dense_grid || get_tile_res(width) < 3 || get_tile_res(height) < 3 ||
      get_tile_res(depth) < 3) {
    return false;
  }
  ...
  int sparse_mem_use = tile_count * sizeof(int) +
                       sparse_width * sparse_height * sparse_depth * channels * sizeof(T);
  int dense_mem_use = width * height * depth * channels * sizeof(T);
        ...
        for (int k = -SPARSE_PAD; k < TILE_SIZE + SPARSE_PAD; ++k) {
          for (int j = -SPARSE_PAD; j < TILE_SIZE + SPARSE_PAD; ++j) {
            for (int i = bound_x.x; i < bound_x.y; ++i) {
              int dense_index = compute_index(i + x, j + y, k + z, width, height, depth) *
                                channels;
