...
class OpenVDBTextureBase {
 public:
  static OpenVDBTextureBase *create_from_grid(openvdb::GridBase::Ptr grid,
                                              const Transform &tfm,
                                              int3 resolution,
                                              int3 index_offset);
 ...
 public:
  ...
  void init(typename T::Ptr &in, const Transform &in_tfm, int3 _resolution, int3 _index_offset)
  {
    ...
    if (grid->hasUniformVoxels() && !grid->empty()) {
      intersector =
          new openvdb::tools::VolumeRayIntersector<T,
                                                   T::TreeType::RootNodeType::ChildNodeType::LEVEL,
                                                   openvdb::math::Ray<float>>(*grid);
    }
  ...
  virtual bool hasUniformVoxels() const
  {
  ...
  typename T::Ptr grid;
  openvdb::tools::VolumeRayIntersector<T,
                                       T::TreeType::RootNodeType::ChildNodeType::LEVEL,
                                       openvdb::math::Ray<float>> *intersector;
