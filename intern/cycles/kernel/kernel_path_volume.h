        ...
        PathRadiance *L)
{
	...
	if(light_sample(kg, light_u, light_v, sd->time, sd->P, NULL, state->bounce, &ls))
	{
	...
	if(sample_all_lights) {
		...
		for(int i = 0; i < kernel_data.integrator.num_all_lights; i++) {
			...
			for(int j = 0; j < num_samples; j++) {
				...
				LightSample ls;
				lamp_light_sample(kg, i, light_u, light_v, ray->P, NULL, &ls);
				...
				if(result == VOLUME_PATH_SCATTERED &&
				   lamp_light_sample(kg, i, light_u, light_v, sd->P, NULL, &ls)) {
		...
		if(kernel_data.integrator.pdf_triangles != 0.0f) {
			...
			for(int j = 0; j < num_samples; j++) {
				...
				LightSample ls;
				light_sample(kg, light_u, light_v, sd->time, ray->P, NULL, state->bounce, &ls);
				...
				/* todo: split up light_sample so we don't have to call it again with new position */
				if(result == VOLUME_PATH_SCATTERED &&
				   light_sample(kg, light_u, light_v, sd->time, sd->P, NULL, state->bounce, &ls)) {
					if(kernel_data.integrator.num_all_lights)
						ls.pdf *= 2.0f;
