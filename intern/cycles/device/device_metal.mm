...
public:
  ...
  : Device(info, stats, profiler, background_)
	...
	public:
		MetalDeviceTask(MetalDevice *device, DeviceTask& task)
		: DeviceTask(task)
		{
...
uint64_t MetalSplitKernel::state_buffer_size(device_memory& /*kg*/, device_memory& /*data*/, size_t num_threads)
{
	return 0;
}
...
int2 MetalSplitKernel::split_kernel_global_size(device_memory& kg, device_memory& data, DeviceTask * /*task*/)
{
...
void device_metal_info(vector<DeviceInfo>& devices)
{
	...
	for(id <MTLDevice> mtl_device in mtl_devices) {
		...
		info.id = string_printf("METAL_%s_%16llu",
								mtl_device.name.UTF8String,
								mtl_device.registryID);
