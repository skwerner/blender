...
ccl_device_inline float eval_Ar(const PSCMap *maps, float theta)
{
	...
	if(maps->center_below_hor) {
		return (theta <= maps->phi_l)
			? (eval_ArC(maps, theta) - eval_ArE(maps, theta))
			: maps->L;
	}
...
{
	theta = check_theta(theta, maps->phi_l);
	const float sin_theta = sinf(theta), sin_theta_sq = sin_theta * sin_theta, cos_theta = safe_sqrtf(1.0f - sin_theta_sq); // sign is positive because theta < phi_l < PI/2
...
ccl_device_inline float eval_Ar_inverse(const PSCMap *maps, float Ar_value)
{
	...
	const float
		theta_max = maps->center_below_hor ? maps->phi_l : M_PI_F,
		theta_result = InverseNSB(maps,
			theta_max, A_frac, 1.0f, Ar_max_value);
	const float result = max(0.0f, min(theta_result, theta_max));

	return result;
}

ccl_device_inline void eval_rmin_rmax(const PSCMap *maps, const float theta, float *rmin, float *rmax)
{
...
ccl_device_inline void eval(const PSCMap *maps, float s, float t, float *x, float *y)
{
	...
	const float si = angle_is_neg ? -(sinf(varphi)) : sinf(varphi),
		...
		* (varphi <= M_PI_F*0.5f ? 1.0f : -1.0f), // note sign correction
		rad = safe_sqrtf(s*(rmax*rmax) + (1.0f - s)*(rmin*rmin)),
		xp = rad * co,
		yp = rad * si;
...
ccl_device_inline float sphere_light_projected_sa_eval(float alpha, float beta, float randu, float randv, float *x, float *y, bool p_use_radial)
{
	PSCMap maps;
	PSCMap_initialize(&maps, alpha, beta, p_use_radial);
