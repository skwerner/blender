...
ccl_device float volume_equiangular_sample(KernelGlobals kg,
...
{
  const float t = ray->t;
  const float delta = dot((ls->P - ray->P), ray->D);
  const float D = safe_sqrtf(len_squared(ls->P  - ray->P) - delta * delta);
  if (UNLIKELY(D == 0.0f)) {
    *pdf = 0.0f;
    return 0.0f;
  }
...
ccl_device float volume_equiangular_pdf(KernelGlobals kg,
                                        ccl_private const Ray *ccl_restrict ray,
                                        ccl_private const LightSample *ls,
  ...
  if (!result.indirect_scatter) {
    ...
    if (1.0f - vstate.rscatter >= sample_transmittance) {
      ...
      if (vstate.direct_sample_method != VOLUME_SAMPLE_EQUIANGULAR) {
        ...
        /* Multiple importance sampling. */
        if (vstate.use_mis) {
          const float equiangular_pdf = volume_equiangular_pdf(kg, ray, ls, new_t);
    ...
    ccl_private VolumeIntegrateResult &result)
{
  ...
  if (vstate.direct_sample_method == VOLUME_SAMPLE_EQUIANGULAR) {
    result.direct_t = volume_equiangular_sample(
        kg, ray, ls, vstate.rscatter, &vstate.equiangular_pdf);
  }
...
#  endif
          ...
          volume_integrate_step_scattering(
              kg, sd, ray, ls, coeff, transmittance, vstate, result);
        }
        else {
          /* Absorption only. */
