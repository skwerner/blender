...
static void create_single_slice(VolumeSlicer *slicer,
                                const float depth,
                                const int axis,
                                const int idx)
{
  const float vertices[3][4][3] = {{{depth, slicer->min[1], slicer->min[2]},
...
static void create_axis_aligned_slices(VolumeSlicer *slicer,
                                      ...
                                      const float view_dir[3])
{
  ...
  const float vertices[8][3] = {{slicer->min[0], slicer->min[1], slicer->min[2]},
  ...
  const int edge_list[8][12] = {{0, 1, 5, 6, 4, 8, 11, 9, 3, 7, 2, 10},
...
static void bind_shader(SmokeDomainSettings *sds,
                        GPUShader *shader,
                        GPUTexture *tex_spec,
                        GPUTexture *tex_tfunc,
                        GPUTexture *tex_coba,
                        bool use_fire,
                        const float min[3],
...
static void unbind_shader(SmokeDomainSettings *sds,
                          GPUTexture *tex_spec,
                          GPUTexture *tex_tfunc,
                          GPUTexture *tex_coba,
                          bool use_fire)
...
static void draw_buffer(SmokeDomainSettings *sds,
                        GPUShader *shader,
                        const VolumeSlicer *slicer,
                        const float ob_sizei[3],
                        const float invsize[3],
                        ...
                        const bool do_fire)
{
  ...
  GPUTexture *tex_tfunc = (sds->use_coba) ? create_transfer_function(TFUNC_COLOR_RAMP, sds->coba) :
                                            NULL;
  ...
  glBufferData(
      GL_ARRAY_BUFFER, sizeof(float) * 3 * num_points, &slicer->verts[0][0], GL_STATIC_DRAW);
...
void draw_smoke_volume(SmokeDomainSettings *sds,
                       Object *ob,
                       const float min[3],
                       const float max[3],
                       const float viewnormal[3])
{
  ...
  const float ob_sizei[3] = {
      1.0f / fabsf(ob->size[0]), 1.0f / fabsf(ob->size[1]), 1.0f / fabsf(ob->size[2])};
  ...
  if (view_aligned) {
    ...
    if (sds->axis_slice_method == AXIS_SLICE_FULL) {
      ...
      axis = (sds->slice_axis == SLICE_AXIS_AUTO) ? axis_dominant_v3_single(viewnormal) :
                                                    sds->slice_axis - 1;
      max_slices = 1;
    }
  ...
  if (sds->vdb && sds->vdb->numeric_display) {
    ...
    if (field[0]) {
      for (int i = 0; i < 3; i++) {
        start_co[i] = sds->p0[i] + (sds->cell_size[i] * sds->res_min[i]) +
                      (sds->cell_size[i] * 0.5f);
      }
          ...
          for (int x = 0; x < sds->res[0]; x++) {
            ...
            if (value >= 0.0005f) {
              ...
              view3d_cached_text_draw_add(
                  coord, text, len, 0, V3D_CACHE_TEXT_LOCALCLIP | V3D_CACHE_TEXT_ASCII, col);
            }
...
#ifdef WITH_SMOKE
static void add_tri(float (*verts)[3],
                    float (*colors)[3],
                    int *offset,
                    float p1[3],
                    float p2[3],
...
static void add_needle(float (*verts)[3],
                       float (*colors)[3],
                       float center[3],
                       float dir[3],
                       float scale,
                       ...
                       int *offset)
{
  ...
  float corners[4][3] = {{0.0f, 0.2f, -0.5f},
...
static void add_streamline(float (*verts)[3],
                           float (*colors)[3],
                           float center[3],
                           float dir[3],
                           float scale,
...
typedef void (*vector_draw_func)(
    float (*)[3], float (*)[3], float *, float *, float, float, int *);
#endif /* WITH_SMOKE */
...
void draw_smoke_velocity(SmokeDomainSettings *domain, float viewnormal[3])
{
  ...
  float xyz[3] = {res_min[0] + fmod(-(float)domain->shift[0] + res_min[0], step_size),
  ...
  int num_points_v[3] = {((float)(res_max[0] - floor(xyz[0])) / step_size) + 0.5f,
  ...
  if (domain->slice_method == MOD_SMOKE_SLICE_AXIS_ALIGNED &&
    ...
    const int axis = (domain->slice_axis == SLICE_AXIS_AUTO) ?
                         axis_dominant_v3_single(viewnormal) :
                         domain->slice_axis - 1;
  ...
  for (float x = floor(xyz[0]); x < res_max[0]; x += step_size) {
    for (float y = floor(xyz[1]); y < res_max[1]; y += step_size) {
      for (float z = floor(xyz[2]); z < res_max[2]; z += step_size) {
        int index = (floor(x) - res_min[0]) + (floor(y) - res_min[1]) * res[0] +
                    (floor(z) - res_min[2]) * res[0] * res[1];

        float pos[3] = {min[0] + ((float)x + 0.5f) * cell_size[0],
                        min[1] + ((float)y + 0.5f) * cell_size[1],
                        min[2] + ((float)z + 0.5f) * cell_size[2]};

        float vel[3] = {vel_x[index], vel_y[index], vel_z[index]};
