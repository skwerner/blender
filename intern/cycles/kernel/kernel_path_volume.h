...
#    endif
  ...
  if (kernel_data.integrator.use_direct_light) {
    ...
    if (light_sample(kg, -1, light_u, light_v, sd->time, sd->P, NULL, state->bounce, &ls)) {
...
#      endif
      ...
      if (kernel_data.integrator.use_direct_light) {
        ...
        light_sample(kg, lamp, light_u, light_v, sd->time, ray->P, NULL, state->bounce, &ls);
        ...
        if (result == VOLUME_PATH_SCATTERED) {
          ...
          if (light_sample(kg, lamp, light_u, light_v, sd->time, sd->P, NULL, state->bounce, &ls)) {
            if (double_pdf) {
              ls.pdf *= 2.0f;
            }
