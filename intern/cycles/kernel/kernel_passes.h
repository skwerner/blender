	...
	int sample, float path_total, float path_total_shaded)
{
	...
	buffer += sample_is_even(kernel_data.integrator.sampling_pattern, sample) ? DENOISING_PASS_SHADOW_B : DENOISING_PASS_SHADOW_A;
...
#  endif
	...
	if(kernel_data.film.pass_adaptive_aux_buffer && kernel_data.integrator.adaptive_threshold > 0.0f) {
