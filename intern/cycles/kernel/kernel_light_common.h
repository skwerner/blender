...
ccl_device float sphere_light_pdf(float3 P, float3 light_p, float radius)
{
  const float3 wc = light_p - P;
...
ccl_device float sphere_light_sample(float3 P, float3 *light_p, float radius, float randu, float randv)
{
  float3 wc = *light_p - P;
  const float dist_squared = len_squared(wc);
  const float sin_theta_max2 = radius * radius / dist_squared;
  ...
  *light_p = *light_p + radius * n_world;
