...
static void rna_SmokeDomain_volume_filepath_get(PointerRNA *ptr, char *value)
{
  SmokeDomainSettings *sds = (SmokeDomainSettings *)ptr->data;
  BLI_strncpy(value, sds->volume_filepath, sizeof(sds->volume_filepath));
}

static int rna_SmokeDomain_volume_filepath_length(PointerRNA *ptr)
{
  SmokeDomainSettings *sds = (SmokeDomainSettings *)ptr->data;
  return strlen(sds->volume_filepath);
}

static void rna_SmokeDomain_volume_filepath_set(PointerRNA *ptr, const char *value)
{
  SmokeDomainSettings *sds = (SmokeDomainSettings *)ptr->data;
...
#  endif
  ...
  RNA_def_property_string_funcs(prop,
                                "rna_SmokeDomain_volume_filepath_get",
                                "rna_SmokeDomain_volume_filepath_length",
                                "rna_SmokeDomain_volume_filepath_set");
  ...
  RNA_def_property_ui_text(
      prop, "Multiple Import", "Try to import similarly named files as an animation");
  RNA_def_property_update(prop, NC_OBJECT | ND_MODIFIER, "rna_Smoke_reset");
  ...
  RNA_def_property_ui_text(prop,
                           "Simplify",
                           "Change level of detail of preview (1 for maximum detail, and larger "
                           "numbers for less detail)");
  RNA_def_property_update(prop, NC_OBJECT | ND_MODIFIER, "rna_Smoke_reset");
}
