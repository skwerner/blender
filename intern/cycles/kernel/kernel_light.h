    ...
    KernelGlobals *kg, int lamp, float randu, float randv, float3 P, LightSample *ls)
{
  ...
  if (type == LIGHT_DISTANT) {
    ...
    if (type == LIGHT_POINT || type == LIGHT_SPOT) {
      ...
      if (radius > 0.0f) {
        ls->pdf = sphere_light_sample(P, &ls->P, radius, randu, randv);
        ls->D = normalize_len(ls->P - P, &ls->t);
        ls->Ng = normalize(ls->P - make_float3(klight->co[0], klight->co[1], klight->co[2]));
      }
      else {
        ls->D = normalize_len(ls->P - P, &ls->t);
        ls->Ng = -ls->D;
    ...
    KernelGlobals *kg, int lamp, float3 P, float3 D, float t, LightSample *ls)
{
  ...
  if (type == LIGHT_DISTANT) {
    ...
    /* compute pdf */
    ls->pdf = sphere_light_pdf(P, lightP, radius);
  }
