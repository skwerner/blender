                                  ...
                                  int buffer_pass_stride)
{
  ...
  if (x < prefilter_rect.z && y < prefilter_rect.w) {
    ...
    float *const in = ((float *)ccl_get_tile_buffer(itile)) +
                      (tile_info->offsets[itile] + y * tile_info->strides[itile] + x) *
                          buffer_pass_stride;
    buffer += ((y - prefilter_rect.y) * (prefilter_rect.z - prefilter_rect.x) +
               (x - prefilter_rect.x)) *
              buffer_pass_stride;
                                       ...
                                       int pass_stride)
{
  ...
  if (x < prefilter_rect.z && y < prefilter_rect.w) {
    kernel_filter_detect_outliers(
        x, y, image, variance, depth, output, prefilter_rect, pass_stride);
  }
}
                                           ...
                                           float k_2)
{
  ...
  if (get_nlm_coords(w, h, r, pass_stride, &rect, &co, &ofs)) {
    kernel_filter_nlm_calc_difference(co.x,
                                      co.y,
                                      co.z,
                                      co.w,
                                      weight_image,
                                ...
                                int f)
{
  ...
  int ofs;
  if (get_nlm_coords(w, h, r, pass_stride, &rect, &co, &ofs)) {
    kernel_filter_nlm_blur(co.x, co.y, difference_image + ofs, out_image + ofs, rect, stride, f);
  }
}
                                       ...
                                       int f)
{
  ...
  if (get_nlm_coords(w, h, r, pass_stride, &rect, &co, &ofs)) {
    kernel_filter_nlm_calc_weight(
        co.x, co.y, difference_image + ofs, out_image + ofs, rect, stride, f);
  }
}
                                         ...
                                         int f)
{
  ...
  int ofs;
  if (get_nlm_coords(w, h, r, pass_stride, &rect, &co, &ofs)) {
    kernel_filter_nlm_update_output(co.x,
                                    co.y,
                                    co.z,
                                    co.w,
                                    difference_image + ofs,
                                             ...
                                             bool use_time)
{
  ...
  if (get_nlm_coords_window(w, h, r, pass_stride, &rect, &co, &ofs, filter_window)) {
    kernel_filter_nlm_construct_gramian(co.x,
                                        co.y,
                                        co.z,
                                        co.w,
                                        t,
                                ...
                                int sample)
{
  ...
  if (x < filter_area.z && y < filter_area.w) {
    ...
    kernel_filter_finalize(
        x, y, buffer, rank, filter_area.z * filter_area.w, XtWX, XtWY, buffer_params, sample);
  }
}
