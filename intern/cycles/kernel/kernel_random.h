...
ccl_device uint sobol_dimension(KernelGlobals *kg, int index, int dimension)
{
	...
	for(uint j = 0; i; i >>= 1, j++) {
		if(i & 1) {
			result ^= kernel_tex_fetch(__rng_lookup, 32*dimension + j);
		}
	}
	return result;
}
                                         ...
                                         int dimension)
{
	...
	if(kernel_data.integrator.sampling_pattern == SAMPLING_PATTERN_PMJ) {
		...
		return __uint_as_float(kernel_tex_fetch(__rng_lookup, index) ^ (tmp_rng & 0x007fffff)) - 1.0f;
	}
#ifdef __CMJ__
#  ifdef __SOBOL__
                                        ...
                                        float *fx, float *fy)
{
	...
	*fy = (float)drand48();
	...
	if(kernel_data.integrator.sampling_pattern == SAMPLING_PATTERN_PMJ) {
		...
		uint tmp_rng = cmj_hash_simple(dimension, rng_hash);
		int index = ((dimension % NUM_PJ_PATTERNS) * NUM_PJ_SAMPLES + sample) * 2;
		*fx = __uint_as_float(kernel_tex_fetch(__rng_lookup, index) ^ (tmp_rng & 0x007fffff)) - 1.0f;
		tmp_rng = cmj_hash_simple(dimension+ 1, rng_hash);
		*fy = __uint_as_float(kernel_tex_fetch(__rng_lookup, index + 1) ^ (tmp_rng & 0x007fffff)) - 1.0f;
		return;
	}
#ifdef __CMJ__
#  ifdef __SOBOL__
