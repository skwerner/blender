...
{
  if (get_geometry()->geometry_type!= Geometry::MESH && get_geometry()->geometry_type != Geometry::VOLUME) {
    return -1.0f;
  }

  Mesh *mesh = static_cast<Mesh *>(get_geometry());

  if (!mesh->has_volume) {
    return -1.0f;
  }

  if (get_geometry()->geometry_type == Geometry::VOLUME) {
    Volume *volume = static_cast<Volume *>(geometry);
    return volume->get_max_density();
  }
  return -1.0f;
}
    ...
    Device *, DeviceScene *dscene, Scene *scene, Progress & /*progress*/, bool bounds_valid)
{
  ...
  dscene->object_max_density.clear_modified();
}
